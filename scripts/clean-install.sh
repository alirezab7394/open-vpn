...
# OpenVPN Clean Installation Script
...
echo -e "${GREEN}=== OpenVPN Clean Installation Script ===${NC}"
...
# 2. Clean directories
echo -e "${BLUE}2. Cleaning directories...${NC}"
sudo rm -rf data/ ssl/ 2>/dev/null || echo "Some directories didn't exist"
mkdir -p data/{pki,clients,logs,db}
mkdir -p ssl
chmod 755 data data/pki data/clients data/logs data/db ssl
...
# 4. Start OpenVPN server first
echo -e "${BLUE}4. Starting OpenVPN server...${NC}"
...
# 5. Initialize OpenVPN
echo -e "${BLUE}5. Initializing OpenVPN configuration...${NC}"
...
# 6. Start OpenVPN-UI
echo -e "${BLUE}6. Starting OpenVPN-UI...${NC}"
...
# 7. Start nginx
echo -e "${BLUE}7. Starting nginx proxy...${NC}"
...
# 10. Create first client for testing
echo -e "${BLUE}10. Creating test client...${NC}"
